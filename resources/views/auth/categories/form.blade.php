<form method="POST" action="{{  isset($category) ? route('categories.update', $category->id) : route('categories.store') }}" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="text" name="name" id="name" value="{{ isset($category) ? $category->name : old('name') }}" class="form-control input-default @error('name') is-invalid @enderror"  autocomplete="name" autofocus />
