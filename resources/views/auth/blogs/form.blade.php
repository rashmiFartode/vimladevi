<form method="POST" action="{{  isset($blog) ? route('blogs.update', $blog->id) : route('blogs.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content" id="content"  class="form-control input-default @error('content') is-invalid @enderror"  autocomplete="content" autofocus cols="6" rows="10" required >{{ isset($blog) ? $blog->content : old('content') }}</textarea>
